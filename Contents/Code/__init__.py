...
import string, os, urllib, copy, zipfile, re#, sys
...
def recurSearch(buscaURL, urlList=0):
	...
	for attempt in range(15):
		try:
			# HTTP.ClearCache()
			# HTTP.ClearCookies()
			elem = HTML.ElementFromURL(buscaURL, cacheTime=300, timeout=60, sleep=1)
...
def searchSubs(data, isTvShow, lang='por'):
	...
	if not ' ' in data['Filename']:
		...
		# if not subUrls and d['Nome'] != d['Nome'].translate(None,"'\"!?:"):
			...
			subUrls = subUrls + list(set(doSearch(d, lang, isTvShow)) - set(subUrls))
		if ' ' in d['Nome'] and MATCH > 1:
		# if not subUrls and ' ' in d['Nome'] and MATCH > 1:
	...
	# if not subUrls and isTvShow:
		...
		# d['Temp'] = d['Temp'] + ' '
		# subUrls = subUrls + list(set(doSearch(d, lang, isTvShow)) - set(subUrls))
	if data['Grupo'] and MATCH > 1:
	# if data['Grupo'] and not subUrls and MATCH > 1:
		# Log("%d subs found - Removing release group" % len(subUrls))
		Log("%d subs found - Adding release group" % len(subUrls))
		# del d['Grupo']
		d['Grupo'] = data['Grupo']
		d['Source'] = ''
		subUrls = subUrls + list(set(doSearch(d, lang, isTvShow)) - set(subUrls))
	if not isTvShow and MATCH > 1:
	# if not subUrls and not isTvShow:
		Log("%d subs found - Removing Year" % len(subUrls))
		d['Ano'] = ''
		subUrls = subUrls + list(set(doSearch(d, lang, isTvShow)) - set(subUrls))
	if data['Source'] and MATCH > 1:
	# if d['Source'] and not subUrls and MATCH > 1:
	...
	if not subUrls:
		...
		for subUrl in subUrls:
			if MATCH == 1:
				...
				break
			if (subUrl in s for s in data['Downloaded']):
			...
			try:
				...
				if legExt == "rar":
					for attempt in range(15):
						try:
							Data.Save('templeg.rar',dload.read())
...
def getReleaseGroup(filename):
	...
	for sep in separators:
		if sep in filename:
			group = string.split(filename,sep)[-1][:-4]
